...
class pupilAnalyses(object):
    ...
    def __init__(self, subject, experiment_name, experiment_nr, project_directory, aliases, sample_rate_new=50):
        ...
        # load data:
        self.cue_locked_array = []
        self.choice_locked_array = []
        self.feedback_locked_array = []
        for alias in aliases:
            self.cue_locked_array.append(self.ho.read_session_data(alias, 'time_locked_cue'))
            self.choice_locked_array.append(self.ho.read_session_data(alias, 'time_locked_choice'))
            if self.experiment == 1:
                self.feedback_locked_array.append(self.ho.read_session_data(alias, 'time_locked_feedback'))
        self.cue_locked_array_joined = np.vstack(self.cue_locked_array)
        self.choice_locked_array_joined = np.vstack(self.choice_locked_array)
        if self.experiment == 1:
            self.feedback_locked_array_joined = np.vstack(self.feedback_locked_array)
        ...
        self.present = np.array(self.parameters_joined['present']) * -self.omissions
        ...
        if self.experiment == 1:
            self.pupil_b_feed = np.array(self.parameters_joined['pupil_b_feed'])
            self.pupil_d_feed = np.array(self.parameters_joined['pupil_d_feed'])
    ...
    def trial_wise_pupil(self):
        ...
        'signal_orientation' : pd.Series(np.array(self.parameters_joined.signal_orientation)),
        ...
        'nr_blinks' : pd.Series(np.array(self.parameters_joined.blinks_nr)),
        }
        data = pd.DataFrame(d)
...
class pupilAnalysesAcross(object):
    ...
    def behavior_normalized(self, prepare=False):
        ...
        if prepare:
            ...
            df.to_csv(os.path.join(self.project_directory, 'figures', 'fig2D_source_data.csv'))
    ...
    def SDT_correlation(self, bin_by='pupil_d', bins=10):
        ...
        for bin_by in ['pupil_d']:
            for y in ['c', 'd']:
                # model_comp = 'bayes'
                model_comp = 'seq'
                fig, rs = self.behavior.SDT_correlation(bin_by=bin_by, n_bins=bins, y1=y, model_comp=model_comp, bin_per='session')
    ...
    def average_pupil_responses(self):
        ...
        for i, s in enumerate(self.subjects):
            ...
            sns.tsplot(kernels_pupil_h[:,xlim_indices], time=step[xlim_indices], condition=conditions, value='fMRI response (% signal change)', color='red', ci=66, lw=1, ls='-', ax=ax)
            conditions = pd.Series(['low pupil'], name='trial type')
            sns.tsplot(kernels_pupil_l[:,xlim_indices], time=step[xlim_indices], condition=conditions, value='fMRI response (% signal change)', color='blue', ci=66, lw=1, ls='-', ax=ax)
            # ax.set_ylim(ylim)
            ax.set_xlim(xlim)
    ...
    def grand_average_pupil_response(self,):
        ...
        for i, s in enumerate(self.subjects):
            ...
            kernel_cue_present.append( bn.nanmean(cue_locked_array[present_ind,:], axis=0) )
            kernel_cue_absent.append( bn.nanmean(cue_locked_array[absent_ind,:], axis=0) )
            kernel_cue_yes.append( bn.nanmean(cue_locked_array[yes_ind,:], axis=0) )
            kernel_cue_no.append( bn.nanmean(cue_locked_array[no_ind,:], axis=0) )
            ...
            kernel_choice_present.append( bn.nanmean(choice_locked_array[present_ind,:], axis=0) )
            kernel_choice_absent.append( bn.nanmean(choice_locked_array[absent_ind,:], axis=0) )
            kernel_choice_yes.append( bn.nanmean(choice_locked_array[yes_ind,:], axis=0) )
            kernel_choice_no.append( bn.nanmean(choice_locked_array[no_ind,:], axis=0) )
        ...
        conditions_data_cue = [[np.vstack(kernel_cue_high), np.vstack(kernel_cue_med), np.vstack(kernel_cue_low)], [np.vstack(kernel_cue_hit), np.vstack(kernel_cue_fa), np.vstack(kernel_cue_miss), np.vstack(kernel_cue_cr)], [np.vstack(kernel_cue_yes), np.vstack(kernel_cue_no)], [np.vstack(kernel_cue_present), np.vstack(kernel_cue_absent)]]
        ...
        conditions_labels = [['high', 'med', 'low'], ['H', 'FA', 'M', 'CR'], ['yes', 'no'], ['present', 'absent']]
        conditions_colors = [['red', 'black', 'blue'], ['orange', 'orange', 'green', 'green'], ['orange', 'green'], ['orange', 'green']]
        conditions_alphas = [[1, 1, 1], [1, 0.5, 0.5, 1], [1, 1], [1, 1]]
        conditions_titles = ['pupil', 'SDT', 'yes', 'present']
    ...
    def SDT_correlation_2(self):
        ...
        for n_bins in [3,4,5,6,7,8,9,10,20]:
        # for n_bins in [20]:
            bins = []
            for i in range(len(self.subjects)):
                inds_s = np.zeros((len(self.pupil_b[(self.subj_idx == i)]), n_bins), dtype=bool)
                trial_nr = 0
                for s in np.unique(self.session):
                    bin_measure = self.pupil_b[(self.subj_idx == i) & (self.session == s)]
            ...
            # compute d' and criterion values:
            varX = np.zeros((len(self.subjects), n_bins))
            varY = np.zeros((len(self.subjects), n_bins))
            varY2 = np.zeros((len(self.subjects), n_bins))
            for i in range(len(self.subjects)):
                for b in range(n_bins):
                    varX[i,b] = self.pupil_b[(self.subj_idx == i)][bins[i][:,b]].mean()
                    if var_Y == 'behaviour':
                        hit_rate = (np.sum(np.array(self.hit[(self.subj_idx == i)])[bins[i][:,b]])) / (float(sum(np.array(self.present[(self.subj_idx == i)])[bins[i][:,b]])))
                        fa_rate = (np.sum(np.array(self.fa[(self.subj_idx == i)])[bins[i][:,b]])) / (float(sum(~np.array(self.present[(self.subj_idx == i)])[bins[i][:,b]])))
                        if hit_rate == 1:
                        ...
                        varY[i,b] = hit_rate_z - fa_rate_z
                        varY2[i,b] = -(hit_rate_z + fa_rate_z) / 2.0
                        # varY[i,b] = hit_rate
                        # varY2[i,b] = fa_rate
            ...
            # plot 1
            ax = fig.add_subplot(111)
            ax.errorbar(varX.mean(axis=0), varY2.mean(axis=0), xerr=sp.stats.sem(varX, axis=0), yerr=sp.stats.sem(varY2, axis=0), fmt='o', markersize=6, color=color2, alpha=1, capsize=0, elinewidth=0.5, markeredgecolor='w', markeredgewidth=0.5)
            ax.set_ylabel('Criterion', color=color2)
            ax.set_xlabel('Baseline pupil (% of max)')
            regression_line = []
            ...
            for i in range(len(self.subjects)):
                ...
                regression_line.append(sp.polyval([m,b,c],x_line))
                r1[i] = r_value
            p1 = 0
            ...
            plt.tight_layout()
            fig.savefig(os.path.join(self.project_directory, 'figures', 'SDT_correlation_2_{}.pdf'.format(n_bins)))
        ...
        for regress_rt in [False]:
            ...
            for c in range(len(conditions_titles)):
                ...
                for time_locked in ['stim_locked', 'resp_locked']:
                    ...
                    # stats:
                    for cc in range(2):
                        myfuncs.cluster_sig_bar_1samp(array=data[cc][:,xlim_indices], x=np.array(step[xlim_indices]), yloc=ylocs[cc], color=conditions_colors[c][cc], ax=ax, threshold=0.05, nrand=5000, cluster_correct=False)
                    ax = ax.twinx()
    ...
    def GLM_betas(self):
        ...
        for b in [0]:
            ...
            betas = np.vstack([np.load(os.path.join(self.project_directory, 'across_data', 'GLM_betas_{}_{}.npy'.format(b,s))) for s in self.subjects])
            MEANS = betas.mean(axis=0)
            SEMS = sp.stats.sem(betas, axis=0)
        
            p1 = sp.stats.ttest_rel(betas[:,0],np.zeros(len(self.subjects)))[1]
            p2 = sp.stats.ttest_rel(betas[:,1],np.zeros(len(self.subjects)))[1]
            p3 = sp.stats.ttest_rel(betas[:,2],np.zeros(len(self.subjects)))[1]
            ...
            fig = plt.figure(figsize=(2,3))
            ax = fig.add_subplot(111)
            ax.bar(ind, MEANS[0:3], width = bar_width, yerr=SEMS[0:3], color='k', alpha=1, edgecolor = 'k', ecolor = 'k', linewidth = 0, capsize = 0, align = 'center')
            ax.set_xticks(ind)
            ax.set_xticklabels(['cue', 'choice', 'box'])
            for i, p in enumerate([p1,p2,p3]):
                ax.text(x=ind[i],y=ax.get_ylim()[1]-((ax.get_ylim()[1] - ax.get_ylim()[0]) / 10.0),s='p = {}'.format(round(p,3)))
            plt.ylabel('beta (a.u.)')
            sns.despine(offset=10, trim=True)
            plt.tight_layout()
    ...
    def mean_slow_drift(self,):
        ...
        plt.fill_between(x, y-baseline_sem_f, y+baseline_sem_f, color='k', alpha=0.25)
        plt.plot(x, y, lw=1, color='k', alpha=1, label="low pass (0.01Hz)",)
        plt.plot(x, func2(x, *popt2), 'k', ls='--', alpha=0.5, label="lin. fit")
        plt.plot(x, func(x, *popt), 'r-', label="exp. fit")
        plt.legend()
        plt.ylabel('Pupil size\n(% signal change)')
        plt.xlabel('Time (s)')
        sns.despine(offset=10, trim=True)
        plt.tight_layout()
        fig.savefig(os.path.join(self.project_directory, 'figures', 'grand_mean_slow.pdf'))
